-- إصلاح مشكلة عدم وجود عمود password
-- إضافة العمود المفقود وتحديث البيانات
...
-- 2. إضافة عمود password إذا لم يكن موجوداً
ALTER TABLE public.users 
ADD COLUMN IF NOT EXISTS password VARCHAR(255);

-- 3. تحديث email من auth.users للمستخدمين الموجودين
UPDATE public.users 
SET email = COALESCE(
...
-- 4. تحديث البيانات للمستخدمين المعروفين
UPDATE public.users 
SET 
    email = CASE 
...
-- 5. التأكد من أن جميع السجلات لديها email
UPDATE public.users 
SET email = COALESCE(
    (SELECT au.email FROM auth.users au WHERE au.id = users.id),
...
-- 6. الآن يمكننا تطبيق القيود بأمان
...
-- 7. إضافة المستخدمين الجدد من auth.users إن لم يكونوا موجودين
