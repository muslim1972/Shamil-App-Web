-- This is a debug version of the function to isolate the error.

-- 1. Drop the existing function
DROP FUNCTION IF EXISTS public.get_user_conversations() CASCADE;

-- 2. Create a simplified version of the function
...
BEGIN
    ...
    SELECT
        ...
        'Debug User'::text AS other_username,
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'get_user_conversations') THEN
        RAISE NOTICE 'Debug function get_user_conversations created successfully.';
    ELSE
        RAISE NOTICE 'Failed to create debug function get_user_conversations.';
    END IF;
END $$;
