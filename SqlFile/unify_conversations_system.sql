-- توحيد نظام المحادثات وإصلاح جميع المشكلات
-- هذا السكربت يضمن توافق النظام الجديد مع التطبيق

-- 1. التحقق من وجود جدول conversations بالبنية الصحيحة
DO $$
BEGIN
    -- إنشاء الجدول بالبنية الصحيحة إذا لم يكن موجوداً
    ...
    -- إضافة عمود participants إذا كان مفقوداً
    IF NOT EXISTS (
...
-- 2. إنشاء جدول messages بالبنية الصحيحة
...
-- 3. حذف جميع الدوال القديمة المتعلقة بالمحادثات
...
-- 4. إنشاء دالة ensure_user_exists بالبنية الصحيحة
...
-- 6. إنشاء دالة create_or_get_conversation_with_user بالبنية الجديدة
...
-- 7. إنشاء دالة get_user_conversations للنظام الجديد
...
-- 8. إنشاء دالة has_user_conversations
...
-- 11. التحقق من وجود المستخدمين الثلاثة وتزامنهم
...
-- 12. إعادة تشغيل وظيفة ensure_user_exists للمستخدمين الثلاثة
...
-- 13. التحقق من عدد السجلات في النهاية
...
-- 14. إعادة تحميل schema cache
SELECT 'تم تحديث جميع الدوال والجداول بنجاح' as status;