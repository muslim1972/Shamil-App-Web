-- =====================================================================================
-- WARNING: MAJOR SCHEMA MIGRATION SCRIPT (v2)
...
-- Step 3: Clean up ALL old RLS policies from ALL related tables.
...
-- This is the policy that caused the error, we drop it now.
DROP POLICY IF EXISTS "Users can view message reads" ON public.message_reads;

-- Step 4: Drop the old 'conversation_members' table using CASCADE and the unused helper function.
-- CASCADE will ensure any other forgotten dependent objects are removed too.
...
-- Messages Policies
CREATE POLICY "Users can view messages in their conversations" ON public.messages FOR SELECT USING (EXISTS (SELECT 1 FROM public.conversations WHERE conversations.id = messages.conversation_id AND auth.uid() = ANY(conversations.participants)));
CREATE POLICY "Users can send messages in their conversations" ON public.messages FOR INSERT WITH CHECK ((sender_id = auth.uid()) AND (EXISTS (SELECT 1 FROM public.conversations WHERE conversations.id = messages.conversation_id AND auth.uid() = ANY(conversations.participants))));

-- Message Reads Policy (New and Corrected)
CREATE POLICY "Users can manage their own message reads" ON public.message_reads FOR ALL USING (auth.uid() = user_id);
...
SELECT 'SUCCESS: Database migration to modern schema (v2) is complete.' as result;
