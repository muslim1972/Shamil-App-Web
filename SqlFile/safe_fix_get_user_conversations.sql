...
-- سكربت آمن لتعديل دالة get_user_conversations لاستبعاد المحادثات المؤرشفة
-- هذا السكربت يتضمن آليات للتحقق والتراجع إذا لزم الأمر
...
BEGIN
    ...
    SELECT 'get_user_conversations' as function_name, current_function as function_definition;
...
CREATE OR REPLACE FUNCTION public.restore_get_user_conversations()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    WHERE function_name = 'get_user_conversations';
...
BEGIN
    ...
    IF function_exists THEN
        RAISE NOTICE 'تم تعديل الدالة بنجاح وإضافة شرط استبعاد المحادثات المؤرشفة';
        RAISE NOTICE 'للتراجع عن التغيير، قم بتنفيذ: SELECT public.restore_get_user_conversations();';
    ELSE
