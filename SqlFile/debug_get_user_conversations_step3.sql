-- This is the third step of debugging to isolate the error.
...
BEGIN
    ...
    SELECT
        ...
        'Debug User Step 3'::text AS other_username,
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'get_user_conversations') THEN
        RAISE NOTICE 'Debug function step 3 get_user_conversations created successfully.';
    ELSE
        RAISE NOTICE 'Failed to create debug function step 3 get_user_conversations.';
    END IF;
END $$;
