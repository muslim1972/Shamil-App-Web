-- Final corrected version of get_user_conversations
-- This version uses the participants array from the conversations table and filters by is_hidden.
...
BEGIN
    ...
    FROM
        ...
        -- Get user settings for the conversation
        JOIN public.user_conversation_settings ucs ON ucs.conversation_id = c.id AND ucs.user_id = p_user_id
        -- Get the other user's data from the auth.users table
        LEFT JOIN auth.users u ON u.id = (array_remove(c.participants, p_user_id))[1]
        -- Get the last message
        LEFT JOIN last_messages lm ON lm.conversation_id = c.id AND lm.rn = 1
    WHERE
        c.participants @> ARRAY[p_user_id] -- Ensure the user is in the conversation
...
DO $$
BEGIN
    RAISE NOTICE 'Function get_user_conversations(UUID) has been corrected and updated successfully.';
END $$;
