...
export default function AuthScreen() {
  ...
  const handleAuth = async () => {
    ...
    // أولاً، اختبر الاتصال بخادم Supabase
    const connectivityTest = await testSupabaseConnectivity();
    console.log('نتيجة اختبار الاتصال:', connectivityTest);
    ...
    if (isLogin) {
      ...
      if (response.error && response.error.message.includes('Cannot connect to server')) {
        ...
        Alert.alert('جاري المحاولة', 'جاري محاولة الاتصال بطريقة بديلة...');

        try {
          response = await signInWithAlternativeClient(email.trim(), password);
        } catch (altError) {
    ...
    if (error) {
      ...
      // إذا فشلت جميع المحاولات، أظهر رسالة أكثر تفصيلاً
      if (error.message.includes('فشل جميع محاولات الاتصال بالخادم')) {
        Alert.alert(
          'فشل الاتصال بالخادم',
          'تعذر الاتصال بخادم Supabase باستخدام جميع الطرق المتاحة. يرجى التحقق من اتصالك بالإنترنت والمحاولة مرة أخرى لاحقاً.',
          [
            { text: 'حسناً', onPress: () => {} }
          ]
        );
